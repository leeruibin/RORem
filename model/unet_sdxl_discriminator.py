...
class UNet2DConditionDiscriminator(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin, PeftAdapterMixin):
    r"""
    ...
    def __init__(
        ...
        # discriminator prediction branch
        self.cls_pred_branch = nn.Sequential(
                # we use 512x512 as input so the input shape is 16x16 which 
    ...
    def forward(
        ...
        predict_output = self.cls_pred_branch(sample)

        logits = predict_output.squeeze(dim=[1, 2, 3])

        class_label = F.softplus(logits)
